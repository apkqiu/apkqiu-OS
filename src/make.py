...
@d.register("os.img", "build/boot/boot.bin, build/boot/loader.bin, build/long/long.bin, build/kernel/kernel.elf")
def make_image(dst, src):
    with open(dst, "wb") as f:
        ...
        f.seek((f.tell() // 512 + 1) * 512)
        print(hex(f.tell()), src[2])
        f.write(open(src[2], "rb").read())
        print(hex(f.tell()))
        f.seek(512 * 64)
        print(hex(f.tell()), src[3])
        f.write(open(src[3], "rb").read())
        print(hex(f.tell()))
        # 扩容至1GB
